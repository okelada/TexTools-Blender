...
from . import op_uv_channel_active_render
...
from . import op_load_bakeset_from_disk
from .settings import tt_settings, prefs
from bpy.app.handlers import persistent
from typing import Union

def get_enum(data, prop_name):
    ...
    if not isinstance(items, (list, tuple)):
        ...
        items = items(data, bpy.context)

    # if not items or len(items) == 0: # The list can be empty
    #     return None,None
    index = data.get(prop_name, None)  # Get the currently stored index

    if index is None:
        default_value = prop.keywords.get("default", 0)
        if isinstance(default_value, int):
            index = default_value
        else:
            # If default value is a string then it's a static enum
            # and we can just return it.
            return default_value,items
    # Ensure index is valid.
    if len(items) > index >= 0:
        return items[index][0],items
    return None,items
...
def timerHandler():
    ...
    try:
        index,items = get_enum(tt_settings(),'uv_channel')
        #if items and tt_settings().uv_channel == '':#in this order
        if items and not index:
            obj = bpy.context.active_object
            if obj and obj.type == 'MESH' and obj.data.uv_layers and len(obj.data.uv_layers) > 0:
                tt_settings().uv_channel = items[0][0] #default, just to kick off the dynamic enum after load or selection changes
    except:
        pass
    return 1.0
...
def load_handler(dummy):
    print("new blendfile loaded")
    settings.sets_update_needed = True
    settings.bakemode_update_needed = True
    utilities_bake.allMaterials = []
    utilities_bake.allMaterialsNames = []

    timerHandler() #to prevent almost all pyrna_enum_to_py warnings

    if not bpy.app.timers.is_registered(timerHandler):
        bpy.app.timers.register(timerHandler, persistent=True)
...
def get_dropdown_uv_common_names():
    all_uvmap_names =[]
    nMaxlayers = 0
    selected_obs = [ob for ob in bpy.context.selected_objects if ob.type == 'MESH']
    for ob in selected_obs:
        ...
        for uvLayer in ob.data.uv_layers:
            obj_uvmap_names.append(uvLayer.name) 
            layerId += 1
            nMaxlayers = max(nMaxlayers,layerId)
        all_uvmap_names.append(obj_uvmap_names)

    for obj_uvmap_names in all_uvmap_names:
        for i in range(len(obj_uvmap_names),nMaxlayers):
            obj_uvmap_names.append(None)

    common_uv_names = []
    for i in range(0,nMaxlayers):
        layer_names = []
        for obj_uvmap_names in all_uvmap_names:
            layer_names.append(obj_uvmap_names[i])
        if layer_names.count(layer_names[0]) == len(layer_names):
            common_uv_names.append(layer_names[0])
        else:
            common_uv_names.append(None)
    return common_uv_names


def get_dropdown_uv_values(self, context):
    options = [] 
    step = 0
    common_uv_names = get_dropdown_uv_common_names()
    for uvCommonName in common_uv_names:
        if common_uv_names[step]:
            hint = f'Change active UV Channel of all selected Objects to {common_uv_names[step]}'  
        else:
            hint = "Check UV channels conformity among objects" 
        options.append((str(step), f"UV{step + 1} {common_uv_names[step] if common_uv_names[step] else '(various names)'}",
                        hint,
                            step))
        step += 1
    return options
...
# 	default = False)
    single_prefix : StringProperty(
    	name="Prefix",
    	description="Prefix to be added in Single mode",
    	default = "singlebake")
    ...
    def draw(self, context):
        ...
        if obj and (obj.mode == 'EDIT' or (bpy.context.selected_objects and obj in bpy.context.selected_objects)):
            if obj.type == 'MESH':
                ...
                if not obj.data.uv_layers or len(obj.data.uv_layers) == 0 or tt_settings().uv_channel == '' :
                    row.operator(op_uv_channel_add.op.bl_idname, text="Add UVMap", icon='ADD')
                else:
                    # UV Channel
                    group = row.row(align=True)

                    if int(tt_settings().uv_channel) > len(obj.data.uv_layers) and len(obj.data.uv_layers) > 0:
                         tt_settings().uv_channel = str(len(obj.data.uv_layers) - 1)
                    ...
                    group.operator(op_uv_channel_active_render.op.bl_idname, text="", icon='RESTRICT_RENDER_OFF')
...
class UI_PT_Panel_Bake(Panel):
    ...
    def draw(self, context):
        ...
        if not (tt_settings().bake_freeze_selection and len(settings.sets) > 0) or settings.sets_update_needed:
            #print("Updating bakesets")
            settings.sets = utilities_bake.get_bake_sets()
            settings.sets_update_needed = False
        ...
        uvc = get_dropdown_uv_values(self,context)
        uvcn = ''
        if len(uvc) > 0 and tt_settings().uv_channel != '':
            uvcn = uvc[int(tt_settings().uv_channel)][1]
        row.operator(op_bake.op.bl_idname, text=f'Bake {count}x - {uvcn} - {prefs().bake_device}', icon_value=icon_get('op_bake'))
        ...
        if settings.bversion >= 3.0:
            ...
            if bpy.context.active_object and bpy.context.active_object.type == 'MESH':
                row.operator(op_texture_preview.op.bl_idname, text=f"Preview Texture ({len(bpy.context.selected_objects)} objects)" ,icon_value=icon_get("op_texture_preview"))
            else:
                row.label(text="Select objects to preview")
        ...
        if len(images) > 0:
            ...
            for image in images:
                ...
                if image == image_background:
                    icon = 'RADIOBUT_ON'
                    if settings.bakemode_update_needed: #try synchro with template_icon_view once at loading
                        for bkmode in op_bake.modes.keys():
                            if "_" + bkmode in image.name:
                                bake_mode = bkmode
                                break
                        bpy.context.window_manager.TT_bake_mode = bake_mode+".bip"
                        settings.bakemode_update_needed = False
        ...
        # Bake Mode
        col.template_icon_view(bpy.context.window_manager, "TT_bake_mode", scale=5.0,scale_popup=preferences.bake_mode_panel_scale)

        if bpy.app.debug_value != 0:
            row = col.row()
            row.label(text=f"--> Mode: '{bpy.context.window_manager.TT_bake_mode}'")
        ...
        if len(settings.sets) > 0:
            ...
            if tt_settings().bake_force == "Single" and len(settings.sets) > 0:
                row.prop(tt_settings(), "single_prefix", text="Bakeset")
                row.operator(op_load_bakeset_from_disk.op.bl_idname,text="",icon='FILE_FOLDER')
...
classes = (
    ...
    op_uv_channel_active_render.op,
